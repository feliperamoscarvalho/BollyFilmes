...
public class FilmesSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public static final int SYNC_INTERVAL = 60 * 720; //60 segundos vezes 720 minutos em 12 horas, para rodar de 12 em 12 horas.É o tempo máximo

    public static final int SYNC_FLEXTIME = SYNC_INTERVAL/3; //é o tempo mínimo necessário para fazer a sincronização
    ...
    public void notify(ItemFilme itemFilme){
        ...
        //Se a configuração de notificação estiver desmarcada, não mostra notificação
        if(!notifyPrefs){
            return;
        }

        //configuração de notificação marcada, vai mostrar a notificação
        ...
        //O stackbuilder é para criar a "pilha fake", quando o usuário clicar em voltar após abrir a acvity detalhe pela notificação,
        //o app será fechado, ao invés de voltar para a activity principal
        ...
        notificationManager.notify(NOTIFICATION_FILMES_ID, builder.build()); //mantendo o id fixo, utiliza sempre a mesma notificação, não gerando várias para o usuário
    ...
    public static Account getSyncAccount(Context context) {
        //método para criar a conta "fake"
